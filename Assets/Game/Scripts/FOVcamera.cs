...
{
    [Serializable]
    public class FOVcamera
    {
        public Camera Camera;                               // optional camera setup, if null the main camera will be used
        [HideInInspector] public float m_OriginalFOV;       // the original fov
        ...
        private void CheckStatus(Camera camera)
        {
            if (camera == null)
            {
                throw new System.Exception("FOVcamera camera is null, please supply the camera to the constructor");
            }
        ...
        public IEnumerator FOVcameraUp()
        {
        ...
        public IEnumerator FOVcameraDown()
        {
            float t = Mathf.Abs((Camera.fieldOfView - m_OriginalFOV) / FOVIncrease);
            while (t > 0)
            {
